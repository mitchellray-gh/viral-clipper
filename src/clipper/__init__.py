...
class ViralityScorer:
    ...
    def find_clips(self, transcript, video_id: str, source_url: str, trend_keyword: str = "") -> list[ClipCandidate]:
        """
    ...
    def _score_batch(self, windows: list[dict], trend_keyword: str, video_id: str) -> list[ClipCandidate]:
        ...
        windows_json = json.dumps([
            {
                ...
                "end": w["end"],
                "duration": w["duration"],
                "text": w["text"][:800]  # truncate very long windows
            }
            for idx, w in enumerate(windows)
        ], ensure_ascii=False, indent=2)

        prompt = f"""You are a YouTube Shorts virality expert. Analyze these transcript clips and score each for virality potential to grow a YouTube channel rapidly.

Trending topic context: "{trend_keyword}"

Transcript windows to score:
{windows_json}

For each window, evaluate:
1. hook_strength (0.0-1.0): Does it start with a strong hook that makes viewers want to watch? Punchy opening statement, surprising claim, or compelling question?
2. emotional_peak (0.0-1.0): Emotional intensity — humor, shock, awe, inspiration, controversy?
3. information_density (0.0-1.0): Contains surprising facts, statistics, revelations, or counterintuitive ideas?
4. completeness (0.0-1.0): Makes sense as a standalone short? Has a clear arc/payoff?
5. trending_relevance (0.0-1.0): How relevant to the trending topic "{trend_keyword}"?
  ...
  ...
        ...
        for item in scored_data:
            ...
            overall = (
                ...
                + comp * weights.get("completeness", 0.15)
